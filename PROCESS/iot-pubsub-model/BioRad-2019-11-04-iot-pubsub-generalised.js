...
exports.handler = async (event) => {
    ...
    console.log(compiledUser)
    for (let index = 0; index < topicBuilder.length; index++) {
        const element = topicBuilder[index];
        // Fetch User for given Topic
        let querytopic = "";
        topic += element;
        if(index == topicBuilder.length - 1) {
            console.log("last");
            querytopic = topic;
        ...
        if(userData != null && userData != '' ) {
            compiledUser = [... new Set(compiledUser.concat(userData))];
        }
        topic +=  "_";
